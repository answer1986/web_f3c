...
<div id="gpsc--products-<?php echo esc_attr( $post_id ); ?>" class="gpsc--product-slider-carousel"
    ...
    // Verificar si el producto pertenece a la categoría 'ofertas'
    if ( has_term( 'ofertas', 'product_cat', $product->get_id() ) ) {
        // Obtener precios
        ...
        if ( $regular_price > 0 && $sale_price > 0 && $sale_price < $regular_price ) {
            ...
            $discountLabel = '<div style="position: absolute; top: 2px; left: 15px; background-color: yellow; padding: 5px 10px; color: black; font-weight: bold; z-index: 10;">-' . $discount_percentage . '%</div>';
        } else {
            $discountLabel = '';  // No mostrar ninguna etiqueta si no hay descuento real
        }
    } else {
        $discountLabel = '';  // No mostrar ninguna etiqueta si el producto no está en 'ofertas'
    }

    echo '<div class="swiper-slide">
            <div class="gpsc--product-image" style="position: relative;">' .
                $discountLabel .
