...
if ( !class_exists("Tumbas_Nav_Menu") ) {
    class Tumbas_Nav_Menu extends Walker_Nav_Menu {
       ...
       /**
        ...
        public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
            ...
            $classes[] = ($this->display_icon($item))?'':' no-icon ';
            ...
            if ( isset($item->apus_text_label) && !empty($item->apus_text_label) ) {
                switch ( $item->apus_text_label ) {
                    case 'label_new':
                        $text_label  = esc_html__('New', 'tumbas');
                        break;

                    case 'label_hot':
                        $text_label  = esc_html__('Hot', 'tumbas');
                        break;

                    case 'label_featured':
                        $text_label  = esc_html__('Featured', 'tumbas');
                        break;
        ...
        public function display_icon($item) {
            if ( $item->apus_icon_image ) {
                $return = '<img class="'.($item->apus_icon_image_hover ? 'has-icon-hover' : '').'" src="'.esc_url($item->apus_icon_image).'" alt="'.esc_attr($item->title).'"/>';
                if ( $item->apus_icon_image_hover ) {
                    $return .= '<img class="icon-hover" src="'.esc_url($item->apus_icon_image_hover).'" alt="'.esc_attr($item->title).'"/>';
                }
                return $return;
...
// icon hover
function tumbas_custom_megamenu($item) {
    $item_id = esc_attr( $item->post_name );
    ?>
    <p class="field-icon-image description description-wide">
        <div>
            <label for="edit-menu-item-icon-image-<?php echo esc_attr($item_id); ?>"><?php esc_html_e( 'Icon Image When Hover:', 'tumbas' ); ?></label>
            <div class="screenshot">
                <?php if ( $item->apus_icon_image_hover ) { ?>
                    <img src="<?php echo esc_url($item->apus_icon_image_hover); ?>" alt="<?php echo esc_attr($item->title); ?>"/>
                <?php } ?>
            </div>
            <input type="hidden" class="upload_image" name="menu-item-apus_icon_image_hover[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr($item->apus_icon_image_hover); ?>">
...
add_action( 'apus-themer-megamenu-after-icon', 'tumbas_custom_megamenu' );

function tumbas_custom_nav_item($menu_item) {
    $fields = array( 'apus_icon_image_hover' );
...
add_filter( 'wp_setup_nav_menu_item', 'tumbas_custom_nav_item' );

function tumbas_custom_nav_update($menu_id, $menu_item_db_id, $args ) {
...
add_action( 'wp_update_nav_menu_item', 'tumbas_custom_nav_update', 10, 3 );
